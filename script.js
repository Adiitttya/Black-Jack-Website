...
const determineWinner = async () => {
    ...
    let text = `
Your card is ${playerHand} with a value of ${playerValue}.
The Dealers card is ${dealerHand} with a value of ${dealerValue}.
${playerValue > dealerValue ? "<em><b>You win!</em>" : "<em><b>Dealer Wins!</em>"}
    `
    showNotice(text)
}
...
const hitDealer = async () => {
    ...
    if (handValue <= 16) {
        ...
        showNotice("Dealer has 21 \n<b>Dealer wins!")
    }
    else if (handValue > 21) {
        showNotice("Dealer bust \n<b>You win!")
    }
    else {
        determineWinner()
    }
...
const hitPlayer = async () => {
    ...
    if (handValue <= 21) {
        ...
        let text = `<b>Bust!</b> \nYour card is ${playerHand} with a value of ${handValue}.`
        showNotice(text)
    }
}
